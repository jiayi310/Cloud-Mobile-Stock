...
class StockTransferHomeScreen extends StatefulWidget {
  const StockTransferHomeScreen({Key? key}) : super(key: key);

  @override
  State<StockTransferHomeScreen> createState() => _StockTransferHomeScreen();
}

class _StockTransferHomeScreen extends State<StockTransferHomeScreen> {
  bool _visible = false;
  List<StockTransfer> StockTransferList = [], StockTransferList_search = [];
                        ...
                        builder: (context, snapshort) {
                          if (snapshort.hasData) {
                            return ListView.builder(
                                itemCount: StockTransferList.length,
  ...
  Future<List<StockTransfer>> getData() async {
    companyid = (await storage.read(key: "companyid"))!;
    if (companyid != null) {
      String response = await BaseClient().get(
          '/StockTransfer/GetStockTransferListByCompanyId?companyId=' +
              companyid);
      List<StockTransfer> _StockTransferList = StockTransferFromJson(response);

      // Sort the collection list by docDate in descending order
      _StockTransferList.sort((a, b) => b.docDate!.compareTo(a.docDate!));

      // If docDate is the same, sort by docNo in descending order
      _StockTransferList.sort((a, b) => b.docNo!.compareTo(a.docNo!));

      setState(() {
        StockTransferList = _StockTransferList;
        StockTransferList_search = _StockTransferList;
        _isLoading = false;
      });
    }
  ...
  void searchQuery(String query) {
    final suggestions = StockTransferList_search.where((sales) {
      final docNo = sales.docNo.toString().toLowerCase();
      final re = sales.remark.toString().toLowerCase() ?? "";
