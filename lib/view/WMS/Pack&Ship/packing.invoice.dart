...
class PackingInvoice extends StatefulWidget {
  final int? selectedSalesId;

  PackingInvoice({Key? key, this.selectedSalesId}) : super(key: key);

  @override
  State<PackingInvoice> createState() => _PackingInvoiceState();
}

class _PackingInvoiceState extends State<PackingInvoice> {
  List<Sales> sales = [];
  List<Sales> salesList_search = [];
  int? selectedSalesId; // Track the selected invoice ID
  String companyid = "";
  ...
  void initState() {
    super.initState();
    selectedSalesId = widget.selectedSalesId;
    myfuture = getData();
  }
              ...
              InkWell(
                onTap: () {
                  if (selectedSalesId != null) {
                    Navigator.pop(context, selectedSalesId);
                    print('selected: $selectedSalesId');
                  }
                },
                child: Container(
  ...
  Widget buildSalesItem(Sales sale, int index) {
    final isSelected = selectedSalesId == sale.docID;

    return GestureDetector(
      onTap: () {
        setState(() {
          selectedSalesId = sale.docID;
        });
      },
      child: Container(
  ...
  Future<List<Sales>> getData() async {
    ...
    if (companyid != null) {
      String response = await BaseClient()
          .get('/Sales/GetSalesListAvailableForPacking?companyId=' + companyid);
